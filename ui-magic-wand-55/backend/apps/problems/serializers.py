...
class ProblemDetailSerializer(ProblemListSerializer):
    ...
    def get_languages(self, obj: Problem) -> list[dict]:
        """What the editor's language picker offers.

        Defaults to everything the sandbox image can actually build and run, so
        the picker cannot offer a language that would be rejected on submit. A
        version may narrow the list -- a problem whose reference solution needs
        64-bit integers has no business offering a language without them -- but
        never widen it past what the judge supports.
        """
        from apps.judging import languages as registry

        catalogue = registry.as_api_list()
        version = self._version(obj)
        if version is None or not version.language_images:
            return catalogue

        allowed = set(version.language_images)
        narrowed = [lang for lang in catalogue if lang["id"] in allowed]
        return narrowed or catalogue
