...
class SubmissionCreateSerializer(serializers.Serializer):
    ...
    def validate_language(self, value: str) -> str:
        """Reject unsupported languages here, not in the sandbox.

        §7.1 wants work rejected at the API before it consumes a judge worker.
        Without this the submission is accepted, queued, dispatched, and only
        then fails -- and the user waits for a verdict that was never possible.
        """
        from apps.judging import languages

        if not languages.is_supported(value):
            supported = ", ".join(sorted(languages.LANGUAGES))
            raise serializers.ValidationError(
                f"{value!r} is not a supported language. Choose one of: {supported}."
            )
        return value
