...
def create_submission(request: Request) -> Response:
    ...
    # Hand off to the isolated judge. Both paths run the submission in the same
    # sandbox container; they differ only in who waits for it.
    #
    # Enqueuing after the transaction commits is deliberate: a worker is fast
    # enough to claim the job before the row is visible, and a job referencing
    # an invisible row fails in a way that looks like data loss.
    if settings.JUDGE_ASYNC:
        from apps.judging.tasks import enqueue

        try:
            enqueue(submission)
        except Exception:  # noqa: BLE001 - broker down must not lose the row
            # §4.11: the submission is already persisted as QUEUED, so it is
            # recoverable. Reporting success here would be a lie; reporting
            # failure would discard work the user has already had accepted.
            logger.exception("Could not enqueue submission %s", submission.id)
            raise ServiceOverloaded(
                "The judge queue is unavailable. Your submission was saved and "
                "will be judged once it recovers.",
                retry_after=30,
            ) from None
    else:
