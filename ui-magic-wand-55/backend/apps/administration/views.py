...
def resolve_tags(slugs: list[str]) -> list[Tag]:
    """Map slugs to Tag rows, creating any that do not exist yet.

    `Tag.objects.filter(slug__in=...)` was used here, which silently dropped
    every slug with no matching row -- on a fresh install that is all of them,
    so problems were created untagged with no error. Authoring a problem is the
    moment a topic tag first appears, so creating on demand is the behaviour the
    authoring UI needs.

    The display name is derived from the slug and can be edited afterwards via
    the tags endpoint.
    """
    resolved = []
    for raw in slugs:
        ...
        tag, _ = Tag.objects.get_or_create(
            slug=slug, defaults={"name": raw.replace("-", " ").strip().title()[:64]}
        )
        resolved.append(tag)
    return resolved
    ...
    try:
        with audited(
            ...
            if tag_slugs is not None:
                problem.tags.set(resolve_tags(tag_slugs))
